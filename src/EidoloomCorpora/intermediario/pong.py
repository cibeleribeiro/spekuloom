...
class Paddle():
    ...
    def draw(self, window_obj):
        pygame.draw.line(window_obj, self.color, (self.pos[0], self.pos[1] - self.height / 2),
                         (self.pos[0], self.pos[1] + self.height / 2), self.width)
...
def check_collision(ball, paddle1, paddle2):
    if ball.pos[0] <= paddle1.width * 2 + ball.radius or ball.pos[0] >= WIDTH - paddle1.width * 2 - ball.radius:
        if (ball.pos[1] in range(paddle1.pos[1] - int(paddle1.height / 2), paddle1.pos[1] + int(paddle1.height / 2)) and
            ball.pos[0] > int(WIDTH / 2)) or (ball.pos[1] in range(paddle2.pos[1] - int(paddle2.height / 2),
                                                                   paddle2.pos[1] + int(paddle2.height / 2)) and
                                              ball.pos[0] < int(WIDTH / 2)):
            if ball.vel[0] < 0:
